...
export const CODE_VULNERABILITIES_EXERCISES: Exercise[] = [
  {
    ...
    hints: ["🌍 Es entregar al cajero un formulario con el dato en el hueco marcado, en vez de dictarle toda la frase: el usuario no puede 'agregar' instrucciones como OR '1'='1'.","La concatenación con comillas convierte al atacante en autor de la sentencia. Las consultas parametrizadas envían el comando SQL y los datos por separado: el input se trata como valor literal.","El snippet vulnerable concatena req.body.email dentro de la query. La corrección usa placeholders ($1) y pasa el valor en un arreglo aparte."],
    stars: 3,
...
const result = await db.query(sql);`,
  ...
  },
  {
    ...
    hints: ["🌍 Es la diferencia entre escribir en la pared con rotulador (textContent: texto inerte) y pegar un cartel con órdenes (innerHTML: el navegador las ejecuta).","innerHTML interpreta etiquetas y atributos: un comentario con <img onerror=\"...\"> se convierte en código real. textContent trata el contenido como texto plano de forma nativa.","Para HTML enriquecido, DOMPurify.sanitize() elimina etiquetas y atributos peligrosos como <script> u onerror antes de insertarlos."],
    stars: 3,
    category: "XSS",
...
container.innerHTML = comment;`,
  ...
  },
  {
    ...
    hints: ["🌍 exec es darle al operador la frase completa por el interfono con el altavoz abierto: cualquier ';' o '|' se convierte en una orden nueva.","exec() abre una shell y userFilename puede encadenar comandos. execFile ejecuta el binario con un arreglo de argumentos, sin pasar por la shell.","Con execFile, metacaracteres como '; rm -rf /' quedan como texto literal del argumento en vez de comandos encadenados."],
    stars: 4,
...
exec(\`convert \${userFilename} output.png\`);`,
  ...
  },
  {
    ...
    hints: ["🌍 Es un conserje que solo escucha el nombre del paquete, no la dirección completa: aunque escriban '../../etc/passwd', solo entiende 'passwd'.","path.basename extrae solo la parte final del nombre, eliminando directorios relativos (../).","La segunda barrera, filePath.startsWith(PUBLIC_DIR), confina la ruta resuelta dentro del directorio permitido."],
    stars: 3,
...
if (!filePath.startsWith(PUBLIC_DIR)) { throw new Error('Acceso no autorizado'); }`,
  },
  {
    ...
    hints: ["🌍 Que muestres tu carné en la puerta no te da derecho a la taquilla de otro: autenticación (quién eres) y autorización (qué te pertenece) son verificaciones distintas.","Autenticarse no implica autorización sobre cualquier ID: hay que verificar que document.ownerId coincida con req.user.id.","La opción segura comprueba `if (!doc || doc.ownerId !== req.user.id) return 403` antes de devolver el documento."],
    stars: 4,
    category: "IDOR / BOLA",
  ...
  },
  {
    ...
    hints: ["🌍 Es como si alguien modificara el manual de la empresa en vez de su propia ficha: al tocar la plantilla maestra, todos los objetos heredan lo inyectado.","Object.create(null) genera un diccionario sin el prototipo global, así que claves como __proto__ no encuentran dónde alterar la cadena de herencia.","Object.freeze(Object.prototype) congela la plantilla maestra para que ninguna fusión de datos la modifique."],
    stars: 4,
...
Object.freeze(Object.prototype);`,
  },
  {
    ...
    hints: ["🌍 Es un formulario en papel con campos ocultos: si el servidor lee TODO sin filtrar, el atacante rellena una línea que nunca mostraste ('rol: admin').","Pasar req.body directo al ORM acepta cualquier campo inyectado. Un z.object estricto define exactamente qué puede actualizar el usuario.","Define UpdateProfileSchema = z.object({ name, bio }) y usa cleanData = UpdateProfileSchema.parse(req.body) en el update."],
    stars: 3,
...
await db.user.update({ where: { id: req.user.id }, data: cleanData });`,
  },
  {
    ...
    hints: ["🌍 Es la diferencia entre leer el papel que acompaña al paquete (JSON.parse: solo datos) y permitir que el paquete se abra solo y ejecute sus instrucciones (eval).","JSON.parse solo construye tipos primitivos y objetos planos: no instancia clases ni ejecuta nada. eval() convierte el payload en código que corre en tu servidor.","Combina JSON.parse con ConfigSchema.safeParse(parsed) para validar la estructura antes de usarla."],
    stars: 4,
...
const result = ConfigSchema.safeParse(parsed);`,
  },
  {
    ...
    hints: ["🌍 Es un guardia que, al ver una pulsera falsa, revisa la misma fila una y otra vez en un bucle sin fin: (a+)+ con una entrada diseñada para fallar congela la CPU.","Los cuantificadores anidados como (a+)+ o (\\w+)+$ provocan backtracking exponencial que bloquea el Event Loop de Node.js.","La opción segura usa validator.isEmail: un validador especializado, lineal por diseño y que nunca se atasca."],
    stars: 4,
    category: "ReDoS",
...
function validateEmail(email: string): boolean {
  ...
  },
  {
    ...
    hints: ["🌍 Es dejar la llave de la oficina bajo el felpudo y publicar una foto del felpudo: una vez que el secreto entra a git, queda en el historial para siempre.","Lee la clave desde process.env.PAYMENT_API_KEY en vez de hardcodearla. El throw al faltar la variable (fail-fast) evita arrancar con configuración rota.","const apiKey = process.env.PAYMENT_API_KEY; if (!apiKey) { throw new Error('PAYMENT_API_KEY no está configurada'); }"],
    stars: 2,
