...
export const REACT_BEST_PRACTICES: Exercise[] = [
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: una biblioteca bien organizada agrupa los libros por tema, no por color de portada. Lo mismo con el código: agrupa por funcionalidad, no por tipo de archivo.\n\nCada feature agrupa sus components, hooks, interfaces y api. Lo transversal (instancia de Axios, stores globales, tipos compartidos) vive en common/. Organizar por feature escala mucho mejor que por tipo técnico: tocar una feature no te obliga a saltar entre 5 carpetas.",
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: el prefijo VITE_ es el candado del buzón: al cliente solo llega la correspondencia que marcaste como pública, no las llaves de la oficina.\n\nSolo las variables con prefijo VITE_ se exponen al cliente (es una medida de seguridad). El alias del bundler (vite.config) y el de TypeScript (tsconfig) deben coincidir; si no, el editor marca errores aunque el build funcione.",
...
"paths": { "@/*": ["[INPUT_3]/*"] }`,
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: front y back firman el mismo contrato: si uno cambia una cláusula y el otro no, la transacción (la app) se rompe en silencio.\n\nEsta interface debe coincidir EXACTAMENTE con ApiResponse.cs del backend: es el puente tipado entre back y front. data es T | null porque cuando hay error no llegan datos, y tiparlo así evita tratar un null como si fuera un objeto.",
...
export const isApiSuccess = <T>(res: ApiResponse<T>): res is ApiResponse<T> & { data: T } =>
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: traducir un DTO es como pasar una receta a otro idioma: los ingredientes son los mismos, solo cambia cómo se nombran.\n\nEl envelope lo cubre ApiResponse<T> genérico; el tipo concreto T se traduce a mano desde el record de C#. Reglas: Guid -> string, DateTime -> string ISO 8601, decimal -> number. Tipar lo que realmente llega evita tratar una fecha como Date cuando en verdad es un string.",
...
export interface Producto {
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: no vas a la oficina de correos por cada carta: dejas un buzón ya configurado en tu puerta. La instancia de Axios es ese buzón.\n\nNunca uses axios directamente: crea una instancia con baseURL, timeout y withCredentials. withCredentials: true es necesario para que el navegador envíe la cookie httpOnly del refresh token; la baseURL viene de la variable de entorno, nunca hardcodeada.",
...
export const api: AxiosInstance = axios.[INPUT_1]({
  ...
  },
  {
    ...
    fileName: "common/api/axios.instance.ts",
    explanationText: "🌍 Ejemplo cotidiano: la secretaria sella y firma cada carta antes de enviarla: el autor no se preocupa de recordar el sello.\n\nEl interceptor lee el token del TokenManager (en memoria) y lo agrega como 'Bearer <token>'. Así ningún servicio tiene que acordarse de adjuntar el header manualmente en cada llamada; si se olvidara uno, ese endpoint fallaría con 401.",
  ...
  },
  {
    ...
    fileName: "common/api/axios.instance.ts",
    explanationText: "🌍 Ejemplo cotidiano: es renovar tu pase de transporte automáticamente cuando caduca, sin hacerte bajar del autobús; y si te dicen \"espere\", esperas en vez de tocar el timbre sin parar.\n\nEl flag _retry evita bucles infinitos de refresh. El 429 corresponde al rate limiter del backend: se respeta el header Retry-After antes de reintentar, en vez de martillar el servidor.",
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: no dejas las llaves de casa bajo el felpudo, donde cualquier curioso las encuentra; las llevas en el bolsillo (memoria).\n\nGuardar el JWT en localStorage lo expone a robo por XSS (cualquier script puede leerlo). Mantenerlo en una variable de módulo (memoria) reduce el riesgo: se pierde al recargar y se recupera vía refresh token.",
...
export const TokenManager = {
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: unwrap abre el sobre y te deja solo la carta: el resto del código no necesita saber que existía un sobre con remitente y sellos.\n\nunwrap centraliza el desempaquetado: los servicios y hooks trabajan directamente con T, sin ver el envelope. El ApiError lleva el message y los errors del backend para mostrarlos tal cual en la UI.",
...
export function unwrap<T>(response: ApiResponse<T>): T {
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: ApiService es la plantilla base de carta que ya trae el sobre y el sello; cada servicio solo rellena el contenido.\n\nLos métodos son 'protected' para que solo las subclases los usen, no cualquiera desde fuera. Cada verbo llama a unwrap, así las subclases devuelven T directamente sin tocar el envelope.",
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: cada departamento tiene su propio buzón con su dirección; el de productos solo recibe y envía lo suyo.\n\nSe exporta una única instancia (singleton) del servicio. Cada método retorna el tipo de dominio (Producto) gracias al unwrap aplicado en la clase base; el componente nunca ve el envelope.",
...
export const productosApi = new [INPUT_2]();`,
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: etiquetas cada caja del almacén con el mismo sistema: 'productos', 'productos·lista', 'productos·detalle'. Así encuentras (o invalidas) una familia entera sin adivinar etiquetas.\n\nUna factory de keys evita errores de tipeo y permite invalidar por jerarquía: invalidar 'all' alcanza listas y detalles. 'as const' fija los tipos literales de la tupla para que TanStack las compare bien.",
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: useQuery es tu alacena: si la comida está fresca (staleTime), no vas al súper; la sirves de lo guardado y solo reaprovisionas cuando caduca.\n\nqueryKey identifica la entrada en cache; queryFn obtiene los datos. staleTime define cuánto tiempo se consideran frescos antes de revalidar en segundo plano (aquí 5 min), evitando peticiones innecesarias.",
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: cuando anotas algo nuevo en la libreta maestra, tachas la copia vieja para que nadie siga leyendo datos caducados.\n\nTras una mutación exitosa, invalidateQueries marca la cache como obsoleta y TanStack la vuelve a pedir. Así la UI refleja el cambio sin refrescar a mano ni recargar la página.",
  ...
  },
  {
    ...
    fileName: "main.tsx",
    explanationText: "🌍 Ejemplo cotidiano: el QueryClient es la central eléctrica de la app: se construye una sola vez y se conecta en la raíz, para que todos los enchufes compartan la misma corriente.\n\nEl QueryClient se crea FUERA del componente para que no se recree en cada render. defaultOptions centraliza el comportamiento (retry, refetchOnWindowFocus) de todas las queries de la app.",
...
root.render(
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: los permisos son llaves con nombre grabado: 'Productos.Vista' y 'Productos.Edición'. Un grabado mal hecho (un typo) significa una llave que no abre ninguna puerta.\n\nEl valor debe ser idéntico al del backend (Permisos.Productos.Vista). 'as const' permite derivar un union type seguro con todos los permisos para tiparlos en toda la app y evitar strings inventados.",
...
export type Permiso = [INPUT_2][keyof typeof PERMISOS];`,
  ...
  },
  {
    ...
    fileName: "common/auth/permisos.ts",
    explanationText: "🌍 Ejemplo cotidiano: ocultar el botón de editar es como no enseñar la llave al visitante; pero la puerta de verdad sigue cerrada y la custodia el backend.\n\nEstas comprobaciones del front son SOLO para UX (ocultar o mostrar botones). La autorización REAL siempre la hace el backend; nunca confíes solo en el cliente para proteger una acción.",
...
export const hasAnyPermission = (userPerms: string[], required: Permiso[]): boolean =>
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: el Context es el tablón de anuncios del edificio: todos leen la misma nota. Si intentas leerlo desde un edificio que no tiene tablón, el hook te lo grita de inmediato.\n\nEl hook useAuth lanza un error si se usa fuera del Provider: esto detecta el bug en desarrollo, en vez de fallar silenciosamente con undefined más adelante y dejarte adivinando.",
...
export function useAuth(): AuthState {
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: ProtectedRoute es la puerta con guardia del salón VIP: sin pase, te acompaña a la salida antes de que veas nada.\n\nCombina useAuth con hasPermission. Si falta el permiso, redirige con 'replace' para no romper el botón de atrás. Recuerda: es protección de UX; el backend valida de verdad cada petición.",
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: el schema de Zod es el pasaporte que define qué datos entran Y de qué tipo son: una sola fuente, sin escribir la lista de requisitos dos veces.\n\nz.infer evita duplicar tipos: el tipo del formulario sale del propio schema. Estas reglas deben reflejar las del backend (FluentValidation), que es la fuente de verdad final de la validación.",
...
const { register, handleSubmit, formState: { errors } } = useForm<FormData>({
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: Zustand es la pizarra compartida de la oficina: escribes una vez y todos los que la miran ven el cambio, sin ir pasando notas de escritorio en escritorio.\n\nset actualiza el estado de forma inmutable. Zustand no necesita Provider: los componentes se suscriben con el hook y solo se re-renderizan si cambia el slice que realmente consumen.",
...
export const useModalStore = [INPUT_1]<ModalState>((set) => ({
  ...
  },
  {
    ...
    explanationText: "🌍 Ejemplo cotidiano: el React Compiler es un chef que solo optimiza recetas puras: si alteras los ingredientes a mitad de la preparación, el chef no puede garantizar el plato.\n\nCon el compiler activo, los useMemo/useCallback manuales casi desaparecen: pero el render debe ser PURO (sin efectos secundarios ni mutar props/estado). No llamar setState durante el render evita bucles.",
